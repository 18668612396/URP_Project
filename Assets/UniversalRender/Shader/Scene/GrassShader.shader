    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            real3 frag (v2f i) : SV_Target
            {
                ...
                float3 normalDir = float3(0.0,1.0,0.0);
                float3 halfDir   = normalize(lightDir + viewDir);
                //点乘计算
                float NdotH = max(0.0,dot(float3(0.0,1.0,0.0),halfDir));//这里假设所有法线朝上
                float NdotV =  max(0.0,dot(float3(0.0,1.0,0.0),lightDir));
                ...
                //Occlusion
                float Occlustion = lerp(1,i.vertexColor.a,_HeightDepth);//把顶点色A通道当作别的草和自己的环境闭塞
                ...
                float3 mainLight = MainLightDiffuse(normalDir,viewDir,i.worldPos,Albedo,0.0,0.04) + specular * _LightColor;
                float3 additionaLight  = AdditionaLightDiffuse(normalDir,viewDir,i.worldPos,Albedo,0.0,0.04);
                float3 indirection = indirectionDiffuse(NdotV,normalDir,0.0,Albedo,1.0,1,0.04);
                
      
                
                //光照合成
                float3 finalRGB = mainLight + additionaLight + indirection;
