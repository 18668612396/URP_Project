    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float fresnel = saturate(1 - pow(NdotV,1));
                float2 scrPos = i.pos.xy / _ScreenParams.xy;//屏幕UV坐标
                half3 SceneColor = SAMPLE_TEXTURE2D(_CameraOpaqueTexture, sampler_CameraOpaqueTexture_linear_clamp, scrPos + normalize(mul(UNITY_MATRIX_V,normalDir)) * _Refraction).rgb;
         
                // return lerp(SceneColor,IndirectionSpecular + IndirectionDiffuse * _BaseColor,fresnel).xyzz;
                return float4(IndirectionSpecular + IndirectionDiffuse * _BaseColor,fresnel);
